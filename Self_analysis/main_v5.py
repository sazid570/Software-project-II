...
def calculate_averages(all_students_marks): ## Changed this function
    ...
    marks_array = np.array(all_students_marks)
    avg_marks = np.mean(marks_array,axis = 0)

    return list(avg_marks)

# Plot the marks and averages, and save to a PDF
def plot_marks(marks, avg_marks, roll_number): ### Changed this function
    n = len(marks)
    ...
    plt.bar(tests, marks,color = col, label="Student's Marks")
    
    # Plot average lines
    for i, avg in enumerate(averages):
        plt.axhline(y=avg,color = col[i], linestyle='--', label=f'Avg {tests[i]}: {avg:.2f}')
